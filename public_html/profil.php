<?php
session_start();
require_once("connex.inc.php");
require_once("auth.php");



$conn = getDatabaseConnection();
$user = current_user();
$id   = current_user_id();
$role = current_role();

/* Historique emploi */
$stid_h = oci_parse($conn, "SELECT type_poste,
                                   TO_CHAR(date_debut,'DD/MM/YYYY') AS date_debut,
                                   TO_CHAR(date_fin,'DD/MM/YYYY') AS date_fin
                            FROM HISTORIQUE_EMPLOI
                            WHERE id_personne = :id
                            ORDER BY date_debut DESC");
oci_bind_by_name($stid_h, ":id", $id);
oci_execute($stid_h);

$emplois = [];
while ($r = oci_fetch_assoc($stid_h)) {
    $emplois[] = $r;
}
oci_free_statement($stid_h);

/* Animaux : pour les métiers animaliers on affiche tous les animaux */
$animaux = [];
if (can_access('animaux')) {
    $stid_a = oci_parse($conn, "SELECT a.nom,
                                       e.nom_usuel AS espece,
                                       en.particularite AS enclos
                                FROM ANIMAL a
                                LEFT JOIN ESPECE e ON a.id_espece = e.id_espece
                                LEFT JOIN ENCLOS en ON a.id_enclos = en.id_enclos
                                ORDER BY a.nom");
    oci_execute($stid_a);
    while ($r = oci_fetch_assoc($stid_a)) {
        $animaux[] = $r;
    }
    oci_free_statement($stid_a);
}

/* Soins : pour métiers animaliers on affiche les derniers soins */
$soins = [];
if (can_access('soins')) {
    $stid_s = oci_parse($conn, "SELECT TO_CHAR(s.date_soin,'DD/MM/YYYY') AS date_soin,
                                       s.type_soin,
                                       s.dosejournaliereSoin,
                                       a.nom AS nom_animal
                                FROM SOINS s
                                JOIN ANIMAL a ON s.id_animal = a.id_animal
                                ORDER BY s.date_soin DESC");
    oci_execute($stid_s);
    while ($r = oci_fetch_assoc($stid_s)) {
        $soins[] = $r;
    }
    oci_free_statement($stid_s);
}

/* Réparations de la personne */
$reparations = [];
if (can_access('travaux')) {
    $stid_r = oci_parse($conn, "SELECT r.nature,
                                       TO_CHAR(r.date_reparation,'DD/MM/YYYY') AS date_rep,
                                       en.particularite,
                                       z.nom_zone
                                FROM REPARATION r
                                JOIN ENCLOS en ON r.id_enclos = en.id_enclos
                                JOIN ZONE z ON en.id_zone = z.id_zone
                                WHERE r.id_personne = :id
                                ORDER BY r.date_reparation DESC");
    oci_bind_by_name($stid_r, ":id", $id);
    oci_execute($stid_r);
    while ($r = oci_fetch_assoc($stid_r)) {
        $reparations[] = $r;
    }
    oci_free_statement($stid_r);
}

/* Boutiques associées */
$boutiques = [];
if (can_access('boutiques')) {
    $stid_b = oci_parse($conn, "SELECT b.nom_boutique, b.type_boutique
                                FROM BOUTIQUE b
                                WHERE b.id_responsable = :id
                                   OR EXISTS (
                                       SELECT 1 FROM TRAVAILLER t
                                       WHERE t.id_boutique = b.id_boutique
                                         AND t.id_personne = :id
                                   )
                                ORDER BY b.nom_boutique");
    oci_bind_by_name($stid_b, ":id", $id);
    oci_execute($stid_b);
    while ($r = oci_fetch_assoc($stid_b)) {
        $boutiques[] = $r;
    }
    oci_free_statement($stid_b);
}

oci_close($conn);
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <title>Mon Profil - Zoo'land Admin</title>
    <link rel="stylesheet" href="style.css">
</head>
<body>
<nav class="admin-nav">
    <a href="accueil.php" class="logo">🌿 Zoo'land Admin</a>
    <div class="nav-actions">
        <span>
            👤 <?= htmlspecialchars(($user['prenom'] ?? '') . ' ' . ($user['nom'] ?? '')) ?>
            <em style="opacity:0.8;">(<?= htmlspecialchars(role_label($role)) ?>)</em>
        </span>
        <a href="accueil.php">Accueil</a>
        <a href="changer_mdp.php">Changer mot de passe</a>
        <a href="logout.php" style="color:#ffcccc;">Déconnexion</a>
    </div>
</nav>

<div class="container">
    <div class="page-header">
        <h1>Mon Profil</h1>
    </div>

    <div class="card" style="margin-bottom:20px;">
        <h2 style="font-size:1rem;margin-bottom:15px;color:var(--primary-color);">Informations générales</h2>
        <table>
            <tr><th>ID</th><td><?= htmlspecialchars($user['id_personne'] ?? '') ?></td></tr>
            <tr><th>Nom</th><td><?= htmlspecialchars($user['nom'] ?? '') ?></td></tr>
            <tr><th>Prénom</th><td><?= htmlspecialchars($user['prenom'] ?? '') ?></td></tr>
            <tr><th>Rôle actuel</th><td><?= htmlspecialchars(role_label($role)) ?></td></tr>
        </table>
    </div>

    <?php if (!empty($emplois)): ?>
    <div class="card" style="margin-bottom:20px;">
        <h2 style="font-size:1rem;margin-bottom:15px;color:var(--primary-color);">Historique d'emploi</h2>
        <table>
            <tr><th>Poste</th><th>Début</th><th>Fin</th></tr>
            <?php foreach ($emplois as $e): ?>
                <tr>
                    <td><?= htmlspecialchars(role_label(strtolower($e['TYPE_POSTE']))) ?></td>
                    <td><?= htmlspecialchars($e['DATE_DEBUT']) ?></td>
                    <td><?= htmlspecialchars($e['DATE_FIN'] ?: 'En cours') ?></td>
                </tr>
            <?php endforeach; ?>
        </table>
    </div>
    <?php endif; ?>

    <?php if (!empty($animaux)): ?>
    <div class="card" style="margin-bottom:20px;">
        <h2 style="font-size:1rem;margin-bottom:15px;color:var(--primary-color);">Animaux visibles</h2>
        <table>
            <tr><th>Nom</th><th>Espèce</th><th>Enclos</th></tr>
            <?php foreach (array_slice($animaux, 0, 10) as $a): ?>
                <tr>
                    <td><?= htmlspecialchars($a['NOM']) ?></td>
                    <td><?= htmlspecialchars($a['ESPECE'] ?? '—') ?></td>
                    <td><?= htmlspecialchars($a['ENCLOS'] ?? '—') ?></td>
                </tr>
            <?php endforeach; ?>
        </table>
    </div>
    <?php endif; ?>

    <?php if (!empty($soins)): ?>
    <div class="card" style="margin-bottom:20px;">
        <h2 style="font-size:1rem;margin-bottom:15px;color:var(--primary-color);">Derniers soins</h2>
        <table>
            <tr><th>Date</th><th>Animal</th><th>Type</th><th>Dose</th></tr>
            <?php foreach (array_slice($soins, 0, 10) as $s): ?>
                <tr>
                    <td><?= htmlspecialchars($s['DATE_SOIN']) ?></td>
                    <td><?= htmlspecialchars($s['NOM_ANIMAL']) ?></td>
                    <td><?= htmlspecialchars($s['TYPE_SOIN']) ?></td>
                    <td><?= htmlspecialchars($s['DOSEJOURNALIERESOIN']) ?></td>
                </tr>
            <?php endforeach; ?>
        </table>
    </div>
    <?php endif; ?>

    <?php if (!empty($reparations)): ?>
    <div class="card" style="margin-bottom:20px;">
        <h2 style="font-size:1rem;margin-bottom:15px;color:var(--primary-color);">Mes réparations</h2>
        <table>
            <tr><th>Date</th><th>Zone</th><th>Enclos</th><th>Nature</th></tr>
            <?php foreach ($reparations as $r): ?>
                <tr>
                    <td><?= htmlspecialchars($r['DATE_REP']) ?></td>
                    <td><?= htmlspecialchars($r['NOM_ZONE']) ?></td>
                    <td><?= htmlspecialchars($r['PARTICULARITE'] ?? '—') ?></td>
                    <td><?= htmlspecialchars(ucfirst($r['NATURE'])) ?></td>
                </tr>
            <?php endforeach; ?>
        </table>
    </div>
    <?php endif; ?>

    <?php if (!empty($boutiques)): ?>
    <div class="card">
        <h2 style="font-size:1rem;margin-bottom:15px;color:var(--primary-color);">Boutiques associées</h2>
        <table>
            <tr><th>Boutique</th><th>Type</th></tr>
            <?php foreach ($boutiques as $b): ?>
                <tr>
                    <td><?= htmlspecialchars($b['NOM_BOUTIQUE']) ?></td>
                    <td><?= htmlspecialchars($b['TYPE_BOUTIQUE']) ?></td>
                </tr>
            <?php endforeach; ?>
        </table>
    </div>
    <?php endif; ?>
</div>
</body>
</html>